class TicketFavUI {
  ...
  renderFavoriteTickets(tickets) {
    ...
    } else if (Object.keys(tickets).length < 2)
      document.querySelector(".btn-clear").classList.add("hidden");
    ...
    Array.from(Object.values(tickets)).forEach((ticket) => {
      const template = TicketFavUI.ticketFavTemplate(ticket);
      fragment += template;
    });
  ...
  static ticketFavTemplate(ticket) {
    return `
    <div class="cart-item">
      <div class="d-flex align-items-center-fav">
        ...
        <div class="d-flex align-items-center-fav ml-auto right">
          <span class="ticket-price-fav">${ticket.price} ${ticket.searchCurrency}</span>
        </div>
      </div>
      ...
      <div class="ticket-destination d-flex align-items-center-fav tops">
        <div class="d-flex align-items-center-fav mr-auto">
          <span class="ticket-city">${ticket.destination_name}</span>
          <i class="medium material-icons">flight_takeoff</i>
        </div>
...
const ticketsFavUI = new TicketFavUI();
